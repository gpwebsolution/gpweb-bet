...
trait CommissionTrait
{
    public static function processAffiliateCommission(User $user, float $betAmount): void
    {
        ...
        $inviter = User::with(['wallet', 'affiliate'])->find($inviterId);
        if (! $inviter) {
            return;
        }
        ...
        if ($inviter->wallet) {
            $inviter->wallet->increment('refer_rewards', $commission);
        }
        ...
        if ($inviter->affiliate) {
            Commission::create([
                'affiliate_id' => $inviter->affiliate->id,
                'user_id' => $user->id,
                'amount' => $commission,
                'type' => 'revshare',
                'status' => 'pending',
            ]);
            $inviter->affiliate->increment('total_commission', $commission);
            $inviter->affiliate->increment('pending_commission', $commission);
        }
    }
}
